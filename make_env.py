...
village_info_dict_of_dicts = {'village0': {'farm': [2, 2, 2, 2, 2, 2], 'mine': [1, 1, 1, 1],
                              'lumber': [1, 1, 1, 1], 'pit': [1, 1, 1, 1], 'inside': [1, 1, 1],
...
class TravianEnv(gym.Env):
    ...
    def res_growth(self, vil_n):
        ...
        for pit in self.X['village' + str(vil_n)].get('pit'):
            ...
            pit_growth += p
        return [farm_growth - self.X['village' + str(vil_n)]['gains'][0], mine_growth, lumber_growth, pit_growth]
    ...
    def is_available_and_rr(self, action_):
        ...
        if action_ == 22 * self.village_n:
            ...
            available = 0
            for_what = self.X['village' + str(integer_)]['waiting_for']
            bui = for_what[0]
            pos = for_what[1]
            lvl = for_what[2]
            if bui == 'inside':
                nazv = ['granary', 'storage', 'main'][pos]
                tc = self.buildings_info[nazv][lvl]['gains'][0]
            else:
                tc = self.buildings_info[bui][lvl]['gains'][0]
            if tc + 2 <= self.res_growth(integer_)[0]:
                time_required = 0
                available = 1
            ...
            if excess_ <= 5:
                ...
                if farm_lvl <= 20:
                    ...
                    time_required = self.buildings_info['farm'].get(farm_lvl).get('costs')[4]*self.boost[integer_]/100
                    resources_required = self.buildings_info['farm'].get(farm_lvl).get('costs')[:4]
                    if all(resources_in[i] >= resources_required[i] for i in range(len(resources_in))):
                        ...
                        if 2 + tc <= self.res_growth(integer_)[0]:
                            available = 1
                ...
                if mine_lvl <= 20:
                    resources_in = self.X['village' + str(integer_)].get('resources')
                    resources_required = self.buildings_info['mine'].get(mine_lvl).get('costs')[:4]
                    time_required = self.buildings_info['mine'].get(mine_lvl).get('costs')[4]*self.boost[integer_]/100
                    if all(resources_in[i] >= resources_required[i] for i in range(len(resources_in))):
                ...
                if lumber_lvl <= 20:
                    resources_in = self.X['village' + str(integer_)].get('resources')
                    resources_required = self.buildings_info['lumber'].get(lumber_lvl).get('costs')[:4]
                    time_required = self.buildings_info['lumber'].get(lumber_lvl).get('costs')[4]*self.boost[integer_]/100
                    if all(resources_in[i] >= resources_required[i] for i in range(len(resources_in))):
                ...
                if pit_lvl <= 20:
                    resources_in = self.X['village' + str(integer_)].get('resources')
                    resources_required = self.buildings_info['pit'].get(pit_lvl).get('costs')[:4]
                    time_required = self.buildings_info['pit'].get(pit_lvl).get('costs')[4]*self.boost[integer_]/100
                    if all(resources_in[i] >= resources_required[i] for i in range(len(resources_in))):
                ...
                if inside_lvl <= 20:
                    resources_in = self.X['village' + str(integer_)].get('resources')
                    resources_required = self.buildings_info[inside_building].get(inside_lvl).get('costs')[:4]
                    time_required = self.buildings_info[inside_building].get(inside_lvl).get('costs')[4]*self.boost[integer_]/100
                    if all(resources_in[i] >= resources_required[i] for i in range(len(resources_in))):
        ...
        if self.is_available_and_rr(action)[0]:
            if action == 22 * self.village_n:
                ...
                if excess_ == 21:
                    ...
                    b_lvl = self.X['village' + str(integer_)][wait_for[0]][wait_for[1]] + 1
                    if wait_for[0] == 'inside':
                        name_for_info = ['granary', 'storage', 'main'][wait_for[1]]
                    ...
                    if self.X['village' + str(integer_)][changes[0]][changes[1]] + 1 <= 20:
    ...
    def reset(self):
        self.X = {'village0': {'farm': [2, 2, 2, 2, 2, 2], 'mine': [1, 1, 1, 1],
                              'lumber': [1, 1, 1, 1], 'pit': [1, 1, 1, 1], 'inside': [1, 1, 1],
                              'resources': [800, 800, 800, 800], 'time_remaining': 0,
                              'gains': [28, 18], 'waiting_for' : 'nothing'}}
        self.village_n = len(self.X)
        self.buildings_info = building_info
        self.gold = gold
        self.action_space = spaces.Discrete(22 * self.village_n + 1)
        self.current_time = 0
        self.granary_capacities = [self.current_capacity_and_boost(i)[0] for i in range(self.village_n)]
